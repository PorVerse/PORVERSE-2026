// middleware.ts - Updated with CSRF + Rate Limiting
...
export async function middleware(req: NextRequest) {
  ...
  // ============================================================================
  // TIER 1: RATE LIMITING + CSRF PROTECTION (CRITICAL SECURITY)
  // ============================================================================

  // 1) Rate Limiting pentru API routes
  if (pathname.startsWith('/api/')) {
    let rateLimitResponse = null

    if (pathname.startsWith('/api/ai/')) {
      rateLimitResponse = await applyRateLimit(req, 'ai', true)
    } else if (pathname.startsWith('/api/auth/')) {
      rateLimitResponse = await applyRateLimit(req, 'auth', false)
    } else if (pathname.startsWith('/api/biometric/')) {
      rateLimitResponse = await applyRateLimit(req, 'biometric', true)
    } else if (pathname.includes('/webhook')) {
      rateLimitResponse = await applyRateLimit(req, 'webhook', false)
    } else {
      rateLimitResponse = await applyRateLimit(req, 'api', false)
    }
  ...
  // 2) CSRF Protection pentru API routes
  if (pathname.startsWith('/api/')) {
    const csrfValid = await csrfProtection(req)
    if (!csrfValid) {
  ...
  // 6) Auth guard (doar în production)
  ...
  // 7) Default response cu security headers

...
export default function EnterpriseLoginPage({
  ...
  // ── Security Functions ──
  const addAuthAttempt = useCallback(
    (success: boolean, method: AuthMethod, error?: string) => {
  ...
  const checkSecurityStatus = useCallback(() => {
    setAuthState(prev => {
      ...
      if (
        prev.security.isLocked &&
        prev.security.lockUntil &&
        now > prev.security.lockUntil
      ) {
        return {
          ...prev,
          security: {
  ...
  const validateForm = useCallback(
    (requirePassword: boolean = true): AuthError | null => {
  ...
  const handleMagicLink = useCallback(
    async (e: React.FormEvent) => {
      ...
      try {
        ...
        const callbackUrl = `${window.location.origin}/${lang}/auth/callback?next=${encodeURIComponent(
          redirectTo
        )}`
  ...
  const formatTimeRemaining = (timestamp: number): string => {
    ...
    if (hours > 0)
      return `${hours}${t.hoursShort} ${minutes % 60}${t.minutesShort}`
    if (minutes > 0)
      return `${minutes}${t.minutesShort} ${seconds % 60}${t.secondsShort}`
  ...
  const isSubmitDisabled =
    authState.loading || !isFormValid || authState.security.isLocked
          ...
          {authState.security.isLocked && authState.security.lockUntil && (
            <div className="mb-6 p-4 bg-red-500/20 border border-red-500/30 rounded-lg">
              ...
              <div className="text-sm text-red-300">
                {t.accountLockedUntil}:{' '}
                {formatTimeRemaining(authState.security.lockUntil)}
              </div>
            </div>
          )}
          ...
          <div className="mb-6">
            <div className="flex bg-white/5 rounded-lg p-1">
              <button
                type="button"
                onClick={() =>
                  setAuthState(prev => ({ ...prev, method: 'password' }))
                }
              ...
              <button
                type="button"
                onClick={() =>
                  setAuthState(prev => ({ ...prev, method: 'magic_link' }))
                }
                className={`flex-1 py-2 px-4 rounded-md text-sm font-medium transition-colors ${
